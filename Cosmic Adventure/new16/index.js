...
function setup()
{
  ...
  selectedSong = gameSongs[0];

  let startButton = new Button(375, 360, 250, 75, "Start Game", function () { menu.page = 1; }, color(130, 0, 180), color(180, 0, 255));
  let tutorialButton = new Button(200, 510, 188, 75, "Tutorial", function () { menu.page = 2; }, color(130, 0, 180), color(180, 0, 255));
  let aboutButton = new Button(490, 510, 300, 75, "About the Game", function () { menu.page = 3; }, color(130, 0, 180), color(180, 0, 255));
  let aiOneButton = new Button(270, 385, 100, 50, "1", function() { selectedAI = 1; selectedSong.setVolume(sliderVal); selectedSong.playMode('restart'); selectedSong.loop(); players = [player]; start(); setInterval(positionRanking, 500); }, color(130, 0, 180), color(180, 0, 255));
  let aiTwoButton = new Button(270, 485, 100, 50, "4", function() { selectedAI = 4; selectedSong.setVolume(sliderVal); selectedSong.playMode('restart'); selectedSong.loop(); players = [player]; start(); setInterval(positionRanking, 500); }, color(130, 0, 180), color(180, 0, 255));
  let aiThreeButton = new Button(455, 385, 100, 50, "9", function() { selectedAI = 9; selectedSong.setVolume(sliderVal); selectedSong.playMode('restart'); selectedSong.loop(); players = [player]; start(); setInterval(positionRanking, 500); }, color(130, 0, 180), color(180, 0, 255));
  let aiFourButton = new Button(455, 485, 100, 50, "19", function() { selectedAI = 19; selectedSong.setVolume(sliderVal); selectedSong.playMode('restart'); selectedSong.loop(); players = [player]; start(); setInterval(positionRanking, 500); }, color(130, 0, 180), color(180, 0, 255));

  let aboutText = new Text(375, 198, 18, color(255, 255, 255), "Cosmic Adventure is a game developed by Evan and some other kid. \nThe game is loosely based on being space adventure / racing game. \nYou can choose to play as one player, or together with a friend locally. \nHUD screens for quick view of each player\'s health, speed and remaining bullets. \nIn the lower left, statistics regarding the game itself are displayed and \nupdated in real time for both players to look at throughout the game. \n\nThe overall goal of the two player game is to pickup the coins around the map \nand proceed to the unlocked end, whilst defending yourself and sneaking around eachother \nand make it to the end first. Whoever makes it first is the winner. \n\nHowever in single player mode, the game takes a largely different approach in which \nthe player races against the set number of enemies, from four set options \nthat are provided to the player before it starts, allowing them to specify the amount of AI \nships that will attempt to seek the end through various checkpoints and path finding. \nFirst person (or AI) to reach the end after three laps is announced the winner.");

  let versionText = new Text(690, 735, 14, color(255, 255, 255), "Version: 1.8.2");
  let aiText = new Text(375, 270, 24, color(255, 255, 255), "Select the amount of AI ships you want to race around the course.")

  let tutorialP1Text = new Text(195, 365, 20, color(255, 255, 255), "Controls: \nW, A, S, D to move around, \nQ and E to strafe left and right, \nSpace to rapidly boost, \nLeft Click to fire projectiles.");
  ...
  let splitButton = new Button(375, 435, 250, 75, "Split Screen", function () { selectedSong.playMode('restart'); selectedSong.loop(); players = [player, player2]; start(); }, color(130, 0, 180), color(180, 0, 255));

  let tutorialP1Image = new ImageGraphic(p1Img, 185, 295, 125, 125);
  ...
  let playAgainButton = new Button(375, 425, 325, 50, "Play Again", function ()
  {
    playing = false;
    paused = false;
    p1Win = false;
  ...
  let unpauseButton = new Button(375, 350, 300, 50, "Return to Game", function () { volumeSlider.hide(); selectedSong.play(); paused = false; playing = true; }, color(130, 0, 180), color(180, 0, 255));
  let songButton = new Button(375, 650, 300, 50, "Switch Music", function() { selectedSong.stop(); selectedSong = gameSongs[gameSongs.findIndex(selectedSong)]; selectedSong.setVolume(sliderVal); selectedSong.playMode('restart'); })
  let songText = new Text(375, 575, 14, color(255, 255, 255), "Current Song: ");


  volumeSlider = createSlider(0, 1, 0.5, 0.1);
  volumeSlider.position(550, 715);
...
function start()
{
  ...
  if(players[1] != player2)
  {
    ...
    {
      players.push(new AIShip(0, i * 25 - 100, 40, 20, aiImg, 5 + i, i / 2 + 1));
    }
  }
